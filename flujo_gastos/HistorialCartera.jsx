...
const HistorialCartera = () => {
  ...
  const handleDataFiltered = (dataFiltrada) => {
    console.log(
      "📊 Datos filtrados recibidos en cartera:",
      dataFiltrada.length,
    );
  ...
  useEffect(() => {
    ...
    const historialConMontoString = historial.map((item) => ({
      ...item,
      monto_estimado_str:
        item.monto_estimado != null ? String(item.monto_estimado) : "",
    }));
  ...
  const onDropVoucher = useCallback(
    (acceptedFiles, id) => {
      ...
      axios
        .post(`${UPDATE_URL}/adjuntarVouchers`, formData, {
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            setHistorial((prev) =>
              prev.map((item) =>
                item.id === id
                  ? {
                      ...item,
                      vouchers: [...(item.vouchers || []), ...nuevosVouchers],
                    }
                  : item,
              ),
            );
            ...
            toast.info(
              <VoucherNotification
                id={id}
                nombreCompleto={gasto?.nombre_completo}
                correo_empleado={gasto?.correo_empleado}
              />,
              {
  ...
  const handleSendVoucher = async (id, correo_empleado) => {
    ...
    try {
      ...
      const response = await axios.post(`${UPDATE_URL}/enviarVouchers`, {
        id,
        correo_empleado,
      });
      if (response.status === 200) {
  ...
  const handleDeleteVoucherFile = async (id, voucherURL) => {
    try {
      const response = await axios.post(`${UPDATE_URL}/eliminarVoucher`, {
        id,
        voucherURL,
      });
      if (response.status === 200) {
  ...
  const getEstadoCarteraClass = (estadoCartera) => {
    switch (estadoCartera) {
      case "Pendiente":
        return "estado-cartera-pendiente";
      case "Anticipo":
        return "estado-cartera-anticipo";
      case "Cancelado":
        return "estado-cartera-cancelado";
      default:
        return "";
    }
  };
  ...
  const handleEstadoCarteraChange = async (id, newEstadoCartera) => {
    try {
      const response = await axios.put(
        `${UPDATE_URL}/actualizarEstadoCartera`,
        {
          id,
      ...
      if (response.status === 200) {
        setHistorial((prev) =>
          prev.map((item) =>
            item.id === id
              ? { ...item, estado_cartera: newEstadoCartera }
              : item,
          ),
        );
    ...
    return (
      <div className="voucher-section">
        ...
        <div className="voucher-actions">
          <FileDropzone id={id} />
          <button className="webcam-button" onClick={() => handleUseWebcam(id)}>
            📷
          </button>
        </div>
      </div>
  ...
  if (errorMessage)
    return (
      <div className="cartera-historial">
        <p className="cartera-error-message">Error: {errorMessage}</p>
      </div>
    );
  ...
  return (
    <div className="cartera-historial">
      ...
      <div
        id="cartera-historial"
        className="cartera-historial-table desplegado"
      >
                  ...
                  <tr key={gasto.id}>
                    ...
                    <td>
                      {gasto.archivos_proveedor ? (
                        JSON.parse(gasto.archivos_proveedor).map((url, i) => {
                          const doc = getDocProveedorInfo(url, i);
                          return (
                            <div key={i}>
                              <a
                                href={url}
                    ...
                    <td className={getEstadoCarteraClass(gasto.estado_cartera)}>
                      <select
                        ...
                        onChange={(e) =>
                          handleEstadoCarteraChange(gasto.id, e.target.value)
                        }
